...
fit()
{
  ...
  TFile* f = new TFile("mcMass502.root", "READ");
  ...
  for(int i = bin40; i < bin50; i++)
  	hh->SetBinContent(i, hh->GetBinContent(i)*scaleSL);  
  	 	
  for(int i = bin50; i < bin60; i++)
  ...
  /*
  TFile* f = new TFile("DoubleMu2012_Summer12_final_11.root", "READ");
  ...
  */
  ...
  eventSelectionamassMu.setRange("SM", 40, 60);
  eventSelectionamassMu.setRange("SU", 70, 120);
  eventSelectionamassMu.setRange("SUP", 60, 120);
  eventSelectionamassMu.setRange("SLP", 10, 40);  
 ...
 //////////// FIT IN SIGNAL REGION, SM /////////
  ...
  TFile* ftop = new TFile("TTFullLept_Summer12_final_11.root", "READ");
