...
TString name = "test";
  double mcSU =  1;//3756;
  double mcSL = 1;//616.227;
  double dataSU = 1;//3185;
  double dataSL = 1;//452;
 ...
 /* TFile* f = new TFile("mcMass30.root", "READ");
...
*/
  TFile* f = new TFile("DoubleMu2012_Summer12_final_4_4.root", "READ");
  ...
  eventSelectionamassMu.setRange("SM", 20, 40);
  eventSelectionamassMu.setRange("SU", 70, 120);
  eventSelectionamassMu.setRange("SP", 40, 120); 	
  eventSelectionamassMu.setRange("INT", 20, 70); 	

  double meanbw = 3655.72;
  double meanb = 1200;
 ...
 //////////// FIT IN SIGNAL REGION, SM /////////
  ...
  TFile* ftop = new TFile("TTFullLept_Summer12_final_4_4.root", "READ");
 ...
 ///////////////
  ...
  sum_norm.fitTo(data, RooFit::Save(kTRUE), RooFit::Range("SL,SP"),RooFit::Extended());
 
  cout<<"////////////// NORMALIZATION /////////////"<<endl;
  cout<<"| | SL | SM | SP |"<<endl;
  cout<<"|bkg | ";  
  fracInt = bkg_norm.createIntegral(eventSelectionamassMu, eventSelectionamassMu, "SL");
  ...
  fracInt = bkg_norm.createIntegral(eventSelectionamassMu, eventSelectionamassMu, "SP");
  cout<<fracInt->getVal() * b_norm.getVal() <<" \\pm " << fracInt->getVal() * b_norm.getError() << "|"<<endl; 
  double U1 = fracInt->getVal() * b_norm.getError() ;  
  
  cout<<"|B.W_pf | ";  
  fracInt = bwxCryBall_norm.createIntegral(eventSelectionamassMu, eventSelectionamassMu, "SL");
  ...
  fracInt = bwxCryBall_norm.createIntegral(eventSelectionamassMu, eventSelectionamassMu, "SP");
  ...
  cout<<"|Sum fit_pf | ";  
  fracInt = sum_norm.createIntegral(eventSelectionamassMu, eventSelectionamassMu, "SL");
  ...
  fracInt = sum_norm.createIntegral(eventSelectionamassMu, eventSelectionamassMu, "SP");
  cout<<fracInt->getVal() * (s_bw_norm.getVal()+b_norm.getVal()/*+ttCont.getVal()*/) <<" \\pm " << sqrt(pow(U1,2)+pow(U2,2)/*+pow(E3,2)*/)  << "|"<<endl;
  
  cout<<"|MC Data | " <<data.sumEntries(0, "SL")<< " | "<< data.sumEntries(0, "SM")<< " | "<<data.sumEntries(0, "SP")<< " | " <<endl;    
TCanvas c3; 
  c3.SetTitle("");
