...
//  RooRealVar frac(name + "frac" ,"tt fraction" , 0.5,0.0, 1.0) ;
  ...
  RooRealVar frac_pf(name + "_frac_pf", "frac_pf", 0.578); 
  ...
  bool doPe = true;
  bool doPeModel = false;
  if(doPe){
 
  	/////// Pseudo experiments before fitting to MC /////
  	...
  	for(int i = 0; i < 100; i++){
  		...
  		RooExtendPdf model_pf(name + "_model_pf", "model_pf",model ,tot ); 
  		for(int i = 0; i < 100; i++){
  			RooDataSet * PD = sum_pf_norm.generate(eventSelectionamassMu, 2000);
  			model_pf.fitTo(*PD, RooFit::Save(kTRUE),RooFit::Range("SM"), RooFit::Extended());
  			double est = tot.getVal();
  			double exp = PD.sumEntries(0, "SM");
  			h->Fill(est-exp);
  			p->Fill((est-exp)/tot.getError());
 ...
 ///////////////
  ...
  cout<<(1-frac_pf.getVal()) * (1-frac.getVal())* tot.getVal() <<" \\pm " <<sqrt(pow(frac.getError()/(1-frac.getVal()),2)+pow(frac_pf.getError()/(1-frac_pf.getVal()) ,2)+pow(tot.getError()/tot.getVal() ,2))* (1-frac.getVal()) * (1-frac_pf.getVal()) * tot.getVal()<< "|"<<endl;
