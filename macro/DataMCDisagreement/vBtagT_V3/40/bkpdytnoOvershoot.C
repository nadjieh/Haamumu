...
*/
 ...
 //////////// FIT IN SIGNAL REGION, SM /////////
  ...
  //RooRealVar frac_dy(name + "frac_dy" ,"dytt fraction" , 0.7, 0.3,1) ;
  RooRealVar frac_dy(name + "frac_dy" ,"dytt fraction" ,0.428571);// 0.428571 -0.147871,0.428571 +0.147871);
  TCanvas C; 
  C.SetTitle("");
  RooPlot * topPdf_frame = eventSelectionamassMu.frame(Title("SM")) ;
  topData.plotOn(topPdf_frame) ;
  topPdf.plotOn(topPdf_frame) ;
  topPdf_frame->Draw();
  C.SaveAs("top.C");
...
//  RooRealVar frac(name + "frac" ,"tt fraction" , 0.5,0.0, 1.0) ;
  ...
  //RooRealVar frac_pf(name + "_frac_pf", "frac_pf", 0.4,0.,1.);
  ...
  RooAddPdf sum_pf_norm(name + "_sum_norm_pf", "Gaussian crystal ball and exponential PDF_pf", RooArgList(fulldy, expo_pf), RooArgList(frac_pf));
  sum_pf_norm.fixCoefRange("SM") ;  
  C.cd();
  sum_pf_norm.plotOn(topPdf_frame, RooFit::LineColor(kYellow));
  topPdf_frame->Draw();
  C.SaveAs("q.C");
  
  RooExtendPdf sum_pf(name + "_sum_pf", "Gaussian crystal ball and exponential PDF_pf",sum_pf_norm ,tot,"SM" );
  fracInt = sum_pf.createIntegral(eventSelectionamassMu,eventSelectionamassMu);
  ...
  sum_pf.plotOn(xFrame_pf, 
        RooFit::Components(fulldy), 
        RooFit::LineStyle(kDashed),RooFit::LineColor(kBlack)) ; 

  xFrame_pf->Draw();


  c2.SaveAs(title+"plot2_dytt.C");
 ...
 ///////////////
  ...
  cout<<"////////////////////////////////"<<endl;
  cout<<"frac_dy: "<<frac_dy.getVal()<<",   frac_pf: "<<frac_pf.getVal()<<",   total: "<< tot.getVal()<<endl;
  cout<<"| | SM |"<<endl;
  cout<<"|BWCB | ";  
  fracInt = bwxCryBall_pf.createIntegral(eventSelectionamassMu, NormSet(eventSelectionamassMu), "SM");
  cout<</*fracInt->getVal()*/(1-frac_dy.getVal()) * frac_pf.getVal() * tot.getVal() <<" \\pm " <<sqrt(pow(frac_dy.getError()/(1-frac_dy.getVal()),2)+pow(frac_pf.getError()/frac_pf.getVal() ,2)+pow(tot.getError()/tot.getVal() ,2))*(1-frac_dy.getVal()) * frac_pf.getVal() * tot.getVal() << "|"<<endl; 
  
  cout<<"|DY tautau | "; 
  fracInt = dytt.createIntegral(eventSelectionamassMu, NormSet(eventSelectionamassMu), "SM"); 
  cout<</*fracInt->getVal()*/(frac_dy.getVal()) * frac_pf.getVal() * tot.getVal() <<" \\pm " <<sqrt(pow(frac_dy.getError()/(frac_dy.getVal()),2)+pow(frac_pf.getError()/frac_pf.getVal() ,2)+pow(tot.getError()/tot.getVal() ,2))*(frac_dy.getVal()) *frac_pf.getVal() * tot.getVal() << "|"<<endl; 

  cout<<"|expo | "; 
  expo_pf.createIntegral(eventSelectionamassMu, NormSet(eventSelectionamassMu), "SM");  
  cout<</*fracInt->getVal()*/(1-frac_pf.getVal()) * tot.getVal() <<" \\pm " <<sqrt(pow(frac_pf.getError()/(1-frac_pf.getVal()) ,2)+pow(tot.getError()/tot.getVal() ,2))*(1-frac_pf.getVal() )* tot.getVal() << "|"<<endl; 
  ...
  sum_norm.fitTo(data, RooFit::Save(kTRUE), RooFit::Range("SLP,SUP"),RooFit::Extended());
  cout<<"////////////// NORMALIZATION /////////////"<<endl;
  cout<<"| | SLP | SM | SUP |"<<endl;
  cout<<"|bkg | ";  
  ...
  fracInt = bkg_norm.createIntegral(eventSelectionamassMu, eventSelectionamassMu, "SUP");
  ...
  fracInt = bwxCryBall_norm.createIntegral(eventSelectionamassMu, eventSelectionamassMu, "SUP");
