...
void mytest(){
  ...
  Double_t lowRange = 20, highRange =70;

  // make a RooRealVar for the observable
  RooRealVar eventSelectionamassMu("eventSelectionamassMu", "eventSelectionamassMu", lowRange, highRange);
  ...
  // nominal model
  RooKeysPdf nominal("nominal","Original keyspdf",eventSelectionamassMu, *data, RooKeysPdf::MirrorBoth,4);
  ...
  Int_t degree = bernsteinCorrection.ImportCorrectedPdf(wks,"nominal","eventSelectionamassMu","data");

  cout << " Correction based on Bernstein Poly of degree " << degree << endl;


  RooPlot* frame = eventSelectionamassMu.frame();
  data->plotOn(frame);
  ...
  if(checkSamplingDist) {
    ...
    int numToyMC = 1000;
    bernsteinCorrection.CreateQSamplingDist(wks,"nominal","eventSelectionamassMu","data",samplingDist, samplingDistExtra, degree,numToyMC);
