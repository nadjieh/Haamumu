...
void test(int seed = 37){
  ...
  //** Nominal histogram
  TH1D* hdata = myPD->createHistogram("hdata", *var,Binning(nbin)); 
  ...
  c2.SaveAs("hist.C");
  //*** Another nominal dataset for Maria
  RooDataSet * myPD2 = sum.generate(eventSelectionamassMu, 1500);
  myPD->SetName("AtoyPD2");
  myPD->SetTitle("A toy PD2");  
  RooKeysPdf pdf21("pdf21","nominal (1)", eventSelectionamassMu, *myPD2, RooKeysPdf::MirrorBoth,1);
  RooKeysPdf pdf22("pdf22","up (2)", eventSelectionamassMu, *myPD2, RooKeysPdf::MirrorBoth,2);
  RooKeysPdf pdf205("pdf205","down (0.5)", eventSelectionamassMu, *myPD2, RooKeysPdf::MirrorBoth,0.5);  
  
  RooPlot * m = eventSelectionamassMu.frame(Title("Two toy PDs"), Bins(nbin));
  myPD->plotOn(m, MarkerColor(kBlack),MarkerStyle(24));
  myPD2->plotOn(m, MarkerColor(kGray+1));  
  TCanvas cm;
  cm.cd();
  m->Draw();
  cm.SaveAs("m.C");
  
  RooPlot * mn = eventSelectionamassMu.frame(Title("Two nominal pdfs")); 
  pdf1->plotOn(mn, LineColor(kRed));
  pdf21->plotOn(mn, LineColor(kRed-7));  
  TCanvas cmn;
  cmn.cd();
  mn->Draw();
  cmn.SaveAs("mn.C");  
    
  RooPlot * mu = eventSelectionamassMu.frame(Title("Two up pdfs"));  
  pdf2->plotOn(mu, LineColor(kGreen));
  pdf22->plotOn(mu, LineColor(kGreen-3));  
  TCanvas cmu;
  cmu.cd();
  mu->Draw();
  cmu.SaveAs("mu.C");    
  
  RooPlot * md = eventSelectionamassMu.frame(Title("Two down pdfs")); 
  pdf05->plotOn(md, LineColor(kBlue));
  pdf205->plotOn(md, LineColor(kAzure+1));  
  TCanvas cmd;
  cmd.cd();
  md->Draw();
  cmd.SaveAs("md.C"); 
    
  RooPlot * ma = eventSelectionamassMu.frame(Title("Two down pdfs and two PDs"), Bins(nbin));    
  myPD->plotOn(ma, MarkerColor(kBlack),MarkerStyle(24));
  myPD2->plotOn(ma, MarkerColor(kGray+1));  
  pdf05->plotOn(ma, LineColor(kBlue));
  pdf205->plotOn(ma, LineColor(kAzure+1));  
  TCanvas cma;
  cma.cd();
  ma->Draw();
  cma.SaveAs("ma.C");   
  
  RooPlot * hn = eventSelectionamassMu.frame(Title("pdfs and the data"), Bins(nbin));    
  myPD->plotOn(hn, MarkerColor(kBlack),MarkerStyle(24));
  pdf1->plotOn(hn, LineColor(kRed));  
  pdf05->plotOn(hn, LineColor(kBlue));
  pdf2->plotOn(hn, LineColor(kGreen)); 
  TCanvas chn;
  chn.cd();
  hn->Draw();
  chn.SaveAs("hn.C");  
