...
void bkg(int seed = 37) {
	...
	if(!binned){
    	...
    	TFile* fsig = new TFile("input_CR.root", "READ");
        ...
        RooPlot * p7 =    eventSelectionamassMu.frame(RooFit::Title("zoomed"),RooFit::Bins(nbin));	
     	bkgData.plotOn(p7,RooFit::MarkerStyle(24));  
     	sum.plotOn(p7,RooFit::FillColor(kWhite), RooFit::LineColor(kRed)); 
	    bkg05.plotOn(p7, RooFit::LineColor(kGreen));    	
    	bkg2.plotOn(p7, RooFit::LineColor(kBlue));      	    
	    TCanvas Can3;     
    	p7->Draw();	  
	    Can3.SaveAs("zoomsum.C");  	
