...
void macro(bool blind = true){
	...
	TFile * f = TFile::Open("rdssignaldata.root");
	TFile * g = TFile::Open("rdscontroldata.root");
	...
	cout <<"KS for nBins "<<hg->GetXaxis()->GetNbins()<<": "<<hf->KolmogorovTest(hg)<<endl;
  	cout<<fsize<<"\t"<<gsize<<endl;
	TFile * F = TFile::Open("DoubleMu2012_Summer12_final_4_4_0"+doblind+".root");
	TTree * tf = (TTree*)F->Get("rds_zbb");
	RDSData * rdsf = new RDSData(tf);;
	TFile * G = TFile::Open("DoubleMu2012_Summer12_final_CR_4_4_0"+doblind+".root");
