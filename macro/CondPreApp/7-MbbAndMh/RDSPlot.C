...
int main(int argc, char** argv) {
    ...
    TH1D * diffMassaa = new TH1D("diffMassaa", "diffMassaa", 1000, 0, 1000);
    TH1D * aptBjet = new TH1D("aptBjet", "aptBjet", 50, 0, 1000);
    TH1D *amassBjet = new TH1D("amassBjet", "amassBjet", 500, 0, 1000);
    ...
    TH2D *amassBjethMass = new TH2D("amassBjethMass", "amassBjethMass", 500, 0, 1000,500, 0, 1000);                               
    int n40 = 0;
    int n40t = 0;
    int n50 = 0;
    int n50t = 0;
    ...
    for (int eventNumber = 0; eventNumber < rds->fChain->GetEntriesFast(); eventNumber++) {
        ...
        if (rds->eventSelectionamassMu > 30 && rds->eventSelectionamassMu < 50) {
            ...
            amassBjethMass->Fill(rds->eventSelectionamassBjet,rds->eventSelectionhMass, weight);
            diffMassaa->Fill(rds->eventSelectiondiffMassaa, weight);
            nTot++;
            if(!((rds->eventSelectionamassBjet - mass) < 10))
				continue;
            nDiff++;
    }
    ...
    amassBjethMass->Write();
    out->Close();
    cout<<"====================================="<<endl;
    cout<<"Ntot "<<nTot<<", NDiff "<<nDiff<<", eff: "<<(double)nDiff/(double)nTot<<endl;  
    cout<<"====================================="<<endl;  
    return 0;
}
