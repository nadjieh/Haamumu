...
public:
    ...
    WSProvider(MeanErr systYield, FixedParams fixed_, double UncSig) : sigNormUnc(UncSig) {
	...
	(w->var("MH"))->setConstant();
        w->factory("y0[5.80131]");
        w->factory("y1[-0.138701]");
        w->factory("y2[0.00187576]");
        //w->factory("PolyVar::signal_norm(MH,{y0,y1,y2})");
        w->factory("FormulaVar::signal_norm(\"@1+(@2*@0)+(@3*TMath::Power(@0,2))\",{MH,y0,y1,y2})");
	...
	w->factory("FormulaVar::mean(\"@1+(@2*@0)\",{MH,m0,m1})");
	w->factory("FormulaVar::sigma_cb(\"@1+(@2*@0)\",{MH,b0,b1})");
	w->factory("FormulaVar::sigma(\"@1+(@2*@0)\",{MH,a0,a1})");
    ...
    void cardMaker() {
        ...
        TString outname = "hamb_shape_ws.txt";
        ...
        if (sigNormUnc != 0)
            myfile << "signal_norm\tlnN\t" << 1 + sigNormUnc << "\t-" << endl;
		...
		myfile << x.first << "\tlnN\t" << x.second.second.first << "\t-" << endl;
	    else
    ...
    void WriteWS() {
        ...
        TFile* f = new TFile("DoubleMu2012_Summer12_final_CR_4_4.root", "READ");
