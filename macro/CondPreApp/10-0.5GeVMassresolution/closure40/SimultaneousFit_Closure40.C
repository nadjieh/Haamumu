...
int main(int argc, char** argv) {
    ...
    fr = simPdfWeighted.fitTo(combDataWeighted, RooFit::Save(), RooFit::SumW2Error(kTRUE));
    {
        ...
        wd40->plotOn(frame2, RooFit::Binning(200));
        m40->plotOn(frame2, RooFit::Binning(200));
        //        m40->plotOn(frame2, Components("tmp_M40"), LineStyle(kDashed));
        //        m40->plotOn(frame2, Components("cball_M40"), LineColor(kRed), LineStyle(kDashed));
        RooHist* hresid40 = frame2->pullHist();
        RooPlot* frame2res = mass->frame(Title("Residual m_{a} = 40 GeV"));
        frame2res->addPlotable(hresid40, "P");
        TCanvas closure;
        closure.Divide(2, 2);
        closure.cd(1);
        ...
        RooPlot* frame22 = mass->frame(Title("40 GeV sample, individual fit"));
        wd40->plotOn(frame22, RooFit::Binning(200));
        m40->plotOn(frame22, RooFit::Binning(200));
        RooHist* hresid402 = frame22->pullHist();
        RooPlot* frame22res = mass->frame(Title("Residual m_{a} = 40 GeV, individual fit"));
        frame22res->addPlotable(hresid402, "P");
        closure.cd(3);
        gPad->SetLeftMargin(0.05);
        ...
        frame22res->Draw();
        closure.SaveAs("closure40_residulaFit.C");
    }
    ...
    combDataWeighted.plotOn(frame1, DataError(RooAbsData::SumW2), Cut("sampleWeighted==sampleWeighted::p30"), RooFit::Binning(200));
    simPdfWeighted.plotOn(frame1, Slice(sampleWeighted, "p30"), ProjWData(sampleWeighted, combDataWeighted), RooFit::Binning(200));
     ...
     * ==============
     */
    RooHist* hresid30 = frame1->pullHist();
    RooPlot* frame1res = mass->frame(Title("Residual m_{a} = 30 GeV"));
    frame1res->addPlotable(hresid30, "P");
    ...
    RooPlot* frame12 = mass->frame(Title("30 GeV sample, individual fit"));
    wd30->plotOn(frame12, RooFit::Binning(200));
    m30->plotOn(frame12, RooFit::Binning(200));
    RooHist* hresid302 = frame12->pullHist();
    RooPlot* frame12res = mass->frame(Title("Residual m_{a} = 30 GeV, individual fit"));
    frame12res->addPlotable(hresid302, "P");
    c->cd(2);
    gPad->SetLeftMargin(0.05);
