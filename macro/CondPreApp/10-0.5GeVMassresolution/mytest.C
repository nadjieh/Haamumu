...
RooFitResult* fitToSignal(TString Mass, int nbin = 50){
		TString name = "test";
    	...
    	//TFile * fsig = new TFile("H2ToH1H1_H1To2Mu2B_mH2-125_mH1-" + Mass + "_LowJetPt10_Summer12_final_" + pf + ".root", "read");
    	TFile * fsig = new TFile("JESD_H2ToH1H1_H1To2Mu2B_mH2-125_mH1_" + Mass + "_Summer12_final_" + pf + ".root", "read");
    	//TFile * fsig = new TFile("JESU_H2ToH1H1_H1To2Mu2B_mH2-125_mH1_" + Mass + "_Summer12_final_" + pf + ".root", "read");  
    	cout<<"---------------->>>>>>>> "  	<<fsig->GetName() <<endl;
    	TTree* hsig = (TTree*) fsig->Get("rds_zbb");
    	RooDataSet data("data_dimu", "data", hsig, eventSelectionamassMu, "");
    	
    	TH1D * hist = (TH1D*)data.createHistogram("hist",eventSelectionamassMu,Binning(nbin));
    	RooRealVar mean("mean", "mean", hist->GetMean(), 0.85*hist->GetMean(),1.15* hist->GetMean());
    	RooRealVar sigma("sigma", "sigma", 0.2,0.,1.);  
    	RooRealVar width("width", "width", hist->GetRMS(), 0.*hist->GetRMS(),1.8*hist->GetRMS());    	
    	RooVoigtian Voig("tmp","",eventSelectionamassMu,mean, width,sigma);    	

    	//RooRealVar mean_cb("mean_cb", "Mean", 50., 40., 60.);
		RooRealVar sigma_cb("sigma_cb", "Width", 2.3, 0., 260.0);
		RooRealVar n("n", "",2,2-0.089,2+0.089);
		RooRealVar alpha("alpha", "", 0.0, 10);
    	RooCBShape CB("cball", "crystal ball", eventSelectionamassMu, mean, sigma_cb, alpha, n);
    	
    	RooRealVar frac("frac","frac",0.46,0.46-0.12,0.46+0.12);
